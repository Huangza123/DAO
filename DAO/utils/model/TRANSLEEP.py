...
class TranSleep(nn.Module):
    def __init__(self, x):
        super(TranSleep, self).__init__()
...
class Stage_Confusion_Estimator(nn.Module):
    def __init__(self, feature_size):
        ...
        self.cls_1st = Classifier(feature_size, 8)
        self.project_att = nn.Linear(8, feature_size)

    def forward(self, x):
        b, l, f, t = x.shape #batch_size=10,channels=1,feature=224,time=8
        #print(x.shape)
        x = x.view(-1, f, t)
        h = self.gap(x).squeeze()#270,224,decrease time
...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, dropout=0.1, max_len=25):
        ...
        #pe = torch.cat([pe, pe.flip(dims=(0,1))[1:]])
...
class MSNN_Feature_Embedding_Two_Way(nn.Module):
    def __init__(self):
        ...
        self.conv_T_0 = conv(1, 4, 50, 32) #args.stride=2
        self.sepconv_T_1 = sepconv_same(4, 16, 15)
        self.sepconv_T_2 = sepconv_same(16, 32, 9)
        self.sepconv_T_3 = sepconv_same(32, 64, 5)

        self.conv_S_0 = conv(1, 4, 200, 32)
        self.sepconv_S_1 = sepconv_same(4, 16, 11)
        self.sepconv_S_2 = sepconv_same(16, 32, 7)
        self.sepconv_S_3 = sepconv_same(32, 64, 3)

        self.gap = nn.AdaptiveAvgPool1d(8)#args.mha_length=8
        self.pe = PositionalEncoding(112, max_len=8)
        self.mha_ff_T = nn.TransformerEncoderLayer(112, 2, dim_feedforward=448)#args.mha_head=2
        self.mha_ff_S = nn.TransformerEncoderLayer(112, 2, dim_feedforward=448)
...
class Context_Encoder(nn.Module):
    def __init__(self, f, h):
        ...
        #if args.lstm_layers == 1: self.biLSTM = nn.LSTM(f, h, num_layers=1, bidirectional=True)#args.lstm_layers=1
        #else: self.biLSTM = nn.LSTM(f, h, num_layers=1, dropout=0.5, bidirectional=True)
        self.biLSTM = nn.LSTM(f, h, num_layers=1, bidirectional=True)
...
if __name__=='__main__':
    x=torch.zeros((512,1,27*960))
    network=TranSleep(x)
    
    x = network.FE(x)#output x shape 10,27,224,8
    print(x.shape)
