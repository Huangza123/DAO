...
def BSL(labels, logits, sample_per_class):
    logits=torch.softmax(logits,dim=1).reshape(-1,5)
    ...
    #print(logits,labels.unique())
    loss = F.cross_entropy(input=logits, target=labels.view(-1))
    return loss
...
def pgd_loss(model,
    ...
    x_adv = x_natural.detach() + 0.001 * torch.randn(x_natural.shape).cuda(1).detach()
    if distance == 'l_inf':
    ...
    loss = BSL(y, model(x_adv)[0], samples_per_cls)
    return loss
...
if __name__=='__main__':
    ...
    samples_per_cls=np.zeros(5)
    for name in os.listdir(dataset_label_path):
        label=np.load(dataset_label_path+name)
        for i in range(label.shape[0]):
            samples_per_cls[int(label[i])]+=1
    ...
    for epoch in range(EPOCH):
        ...
        if (epoch) % 10==0:
            ...
            if (precision+recall+f1+mcc+acc)>best_metrics:
                ...
                torch.save(net,'./ATBSL/save_model/'+str(epoch)+'atbsl_model.pth')
            f=open('./ATBSL/log/atbsl_log.txt','a')
